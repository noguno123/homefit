...
class AudioPlayer {
  constructor(audioElement, statusElement, volumeElement, toggleButton) {
  ...
  initialize() {
    this.setVolume(Number(this.volumeElement.value));
    this.updateStatus();
  }
  ...
  play() {
    ...
    this.audio.play().catch(() => {
      this.statusElement.textContent = "BGM 재생 대기";
    });
  }
  ...
  setVolume(value) {
    ...
    this.audio.volume = safeValue / 100;
    this.isMuted = safeValue === 0;
    this.audio.muted = this.isMuted;
    this.updateStatus();
  }

  updateStatus() {
    const statusText = this.isMuted ? "BGM 꺼짐" : `BGM 켜짐 · ${Math.round(this.audio.volume * 100)}%`;

    this.statusElement.textContent = statusText;
    this.toggleButton.textContent = this.isMuted ? "켜기" : "음소거";
  }
}
...
const dom = {
  ...
  bgmToggleButton: document.querySelector('[data-action="toggle-bgm"]'),
...
const timer = new CountdownTimer(renderTimer, handleTimerComplete);
const bgmPlayer = new AudioPlayer(dom.bgmAudio, dom.bgmStatus, dom.bgmVolume, dom.bgmToggleButton);
...
function renderVideo(videoPath) {
  dom.videoArea.innerHTML = `
    <video controls playsinline preload="metadata" loop muted autoplay>
...
function handleTimerComplete() {
  const isLastWorkout = appState.currentWorkoutIndex === appState.routine.length - 1;
...
function bindEvents() {
  ...
  dom.bgmVolume.addEventListener("input", (event) => {
    bgmPlayer.setVolume(Number(event.target.value));
  });
  ...
  document.addEventListener("click", (event) => {
    ...
    if (action === "toggle-bgm") bgmPlayer.toggleMute();
