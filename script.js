...
class RoutineRecommender {
  ...
  recommend(preferences) {
    ...
    return this.fitToTime(ranked.map((item) => item.exercise), preferences.availableMinutes);
  }
  ...
  fitToTime(exercises, availableMinutes) {
    const targetSeconds = availableMinutes * 60;
    const selected = [];
    let totalSeconds = 0;

    exercises.forEach((exercise) => {
      if (selected.length < 5 && totalSeconds + exercise.duration <= targetSeconds + 90) {
        selected.push(exercise);
        totalSeconds += exercise.duration;
      }
    });

    return selected.length ? selected : exercises.slice(0, 3);
  }
}
  ...
  return `
    <article class="exercise-card">
      <div class="card-head">
        <div>
          <h3>${exercise.name}</h3>
          <p>${getReason(exercise, appState.preferences)}</p>
        </div>
        <span class="badge">${exercise.difficulty}</span>
      </div>
      <div class="meta-row">
        <span>예상 ${formatDuration(exercise.duration)}</span>
        <span>${exercise.category.map((item) => labels[item]).join(" · ")}</span>
      </div>
      <p><strong>주의사항</strong> ${exercise.caution}</p>
    </article>
  `;
}
...
function renderSelectionSummary(preferences, routine) {
  ...
  const totalMinutes = Math.ceil(routine.reduce((sum, exercise) => sum + exercise.duration, 0) / 60);

  dom.selectionSummary.textContent =
    `${labels[preferences.goal]} 목표 · ${preferences.availableMinutes}분 가능 · ${conditionText} 기준으로 약 ${totalMinutes}분 루틴을 추천했어요.`;
}

function renderWorkout() {
  const exercise = appState.routine[appState.currentWorkoutIndex];

  dom.completionMessage.hidden = true;
...
function moveWorkout(direction) {
  ...
  if (nextIndex >= appState.routine.length) {
    timer.pause();
    dom.completionMessage.hidden = false;
    return;
  }
